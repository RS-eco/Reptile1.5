...
#' ### Calculate and plot species richness from raw data ###
...
p1 <- gard_reptiles_dist %>% group_by(x,y) %>% summarise(sr=sum(occ)) %>% 
...
p2 <- gard_reptiles_dist %>% filter(Group == "lizard") %>% group_by(x,y) %>% summarise(sr=sum(occ)) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=sr)) + geom_sf(data=outline, fill="transparent", colour="black") + 
  ...
  theme(plot.background = element_rect(fill = "transparent"), 
        ...
        strip.text = element_text(size=14),
        legend.box.background = element_rect(fill = "transparent", colour=NA))
p3 <- gard_reptiles_dist %>% filter(Group == "snake") %>% group_by(x,y) %>% summarise(sr=sum(occ)) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=sr)) + geom_sf(data=outline, fill="transparent", colour="black") + 
  ...
  theme(plot.background = element_rect(fill = "transparent"), 
        ...
        strip.text = element_text(size=14),
        legend.box.background = element_rect(fill = "transparent", colour=NA))
p4 <- gard_reptiles_dist %>% filter(Group == "turtle") %>% group_by(x,y) %>% summarise(sr=sum(occ)) %>% 
  ggplot() + geom_tile(aes(x=x, y=y, fill=sr)) + geom_sf(data=outline, fill="transparent", colour="black") + 
...
ggsave("figures/FigureS1.png", dpi=600, width=11.5, height=5)

#' => Compare to Roll paper!!!