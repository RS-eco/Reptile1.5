#' ---
#' title: "Relationship of species richness with temperature and precipitation across RCPs for year 2080"
#' author: "RS-eco"
#' ---
...
f_labels <- data.frame(rcp = dat %>% filter(year == 2080) %>% 
                         ungroup() %>% select(rcp) %>% unique() %>% unlist(), 
                       label = c("a) 2080", "b) 2080", "c) 2080"))

dat_sub <- dat %>% filter(rcp != "EWEMBI") %>% filter(year == 2080)
p1 <- dat_sub %>% ggplot() + geom_tile(aes(x=bio1, y=bio12/100, fill=sr)) + 
  ...
  scale_y_continuous(limits=c(min(dat_sub$bio12)/100-1, max(dat_sub$bio12)/100), 
                     ...
                     labels=c(0, 2000, 4000, 6000, 8000, 10000), expand=c(0,0)) + 
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
  geom_text(x = -20, y = 110, aes(label = label), data = f_labels, size=5, hjust=0) + 
...
p2 <- dat_wide %>% filter(rcp == "RCP2.6") %>% ggplot() + 
  ...
  scale_y_continuous(limits=c(min(dat_sub$bio12)/100-1, max(dat_sub$bio12)/100), 
                     ...
                     labels=c(0, 2000, 4000, 6000, 8000, 10000), expand=c(0,0)) + 
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
  annotate("text", x = -20, y = 110, label = "d) 2080 - 1995", size=5, hjust=0) + 
...
p3 <- dat_wide %>% filter(rcp == "RCP6.0") %>% ggplot() + 
  ...
  scale_fill_gradientn(name="Richness \nchange",  colours=redwhiteblue, 
                       values=col_val, limits=lim_map) + 
  annotate("text", x = -20, y = 110, label = "e) 2080 - 1995", size=5, hjust=0) + 
  scale_y_continuous(limits=c(min(dat_sub$bio12)/100-1, max(dat_sub$bio12)/100), 
                     breaks=c(0, 20, 40, 60, 80, 100), 
                     labels=c(0, 2000, 4000, 6000, 8000, 10000), expand=c(0,0)) + 
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
  coord_fixed(ratio=0.5) + theme_bw() + theme(legend.background = element_blank(), plot.title = element_text(size=14, face="bold"),
...
p4 <- dat_wide %>% filter(rcp == "RCP8.5") %>% ggplot() + 
  ...
  scale_fill_gradientn(name="Richness \nchange",  colours=redwhiteblue, 
                       values=col_val, limits=lim_map) + 
  annotate("text", x = -20, y = 110, label = "f) 2080 - 1995", size=5, hjust=0) + 
  scale_y_continuous(limits=c(min(dat_sub$bio12)/100-1, max(dat_sub$bio12)/100), 
                     breaks=c(0, 20, 40, 60, 80, 100), 
                     labels=c(0, 2000, 4000, 6000, 8000, 10000), expand=c(0,0)) + 
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
...
p3 <- dat_wide %>% filter(rcp == "RCP2.6") %>% ggplot() + 
  ...
  scale_y_continuous(limits=c(min(dat_sub$bio12)/100-1, max(dat_sub$bio12)/100), 
                     ...
                     labels=c(0, 2000, 4000, 6000, 8000, 10000), expand=c(0,0)) + 
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
  annotate("text", x = -20, y = 110, label = "g) 2080 - 1995", size=5, hjust=0) + 
  labs(x="Annual mean temperature (°C)", y="Annual precipitation (mm)") + 
  coord_fixed(ratio=0.5) + theme_bw() + theme(legend.background = element_blank(), 
                     plot.title = element_text(size=14, face="bold"),
...
p4 <- dat_wide %>% filter(rcp == "RCP6.0") %>% ggplot() + 
  ...
  scale_y_continuous(limits=c(min(dat_sub$bio12)/100-1, max(dat_sub$bio12)/100), 
                     ...
                     labels=c(0, 2000, 4000, 6000, 8000, 10000), expand=c(0,0)) + 
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
  annotate("text", x = -20, y = 110, label = "h) 2080 - 1995", size=5, hjust=0) + 
...
p5 <- dat_wide %>% filter(rcp == "RCP8.5") %>% ggplot() + 
  ...
  scale_x_continuous(limits=c(min(dat_sub$bio1)-2, max(dat_sub$bio1)), expand=c(0,0)) + 
  annotate("text", x = -20, y = 110, label = "i) 2080 - 1995", size=5, hjust=0) + 
  labs(x="Annual mean temperature (°C)", y="") + 
  coord_fixed(ratio=0.5) + theme_bw() + theme(legend.background = element_blank(), 
                     plot.title = element_text(size=14, face="bold"),
                     axis.text.x = element_text(size=12), axis.title.x = element_text(size=14),
...
ggsave("figures/FigureS35.png", width=11, height=11, dpi=300, bg="transparent")
