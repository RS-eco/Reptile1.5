#' ---
#' title: "Relationship of summed probability with temperature and precipitation"
#' author: "RS-eco"
#' ---
...
library(ggpmisc); library(tagger)
...
dat1 <- inner_join(sr_all, climatedata) %>% 
  mutate(value = round(bio1*2, digits= 0)/2) %>% 
  group_by(year, value) %>%
...
p1 <- dat %>% ggplot(aes(x=value, y=sr, lty=year)) + geom_line(colour="black") + 
  ...
  labs(x="", y="Summed probability") + scale_linetype(name="") + 
  scale_y_continuous(expand=expansion(mult=0.02)) + 
  scale_x_continuous(expand=expansion(mult=0.02)) +
  scale_fill_manual(name="",values=c("#1B9E77", "#D95F02", "#7570B3")) + 
...
f_labels <- data.frame(year = unique(dat$year), 
                       label = c("c) 1995", "d) 2080 RCP6.0"))

p2 <- ggplot(data=dat) + geom_tile(aes(x=bio1, y=bio12/100, fill=sr)) + 
  facet_wrap(. ~ year) + coord_fixed(ratio=0.5) + 
  scale_fill_gradientn(name="Summed\nprobability", colours=bluered) + 
  ...
  theme_bw() + theme(strip.background = element_blank(), strip.text = element_blank(),
                     ...
                     plot.margin=unit(c(0,0.2,0.4,0.2),"mm"))

unique(dat$year)
pres <- dat %>% filter(year == "1995") %>% ungroup() %>% select(-c(year))
fut <- dat %>% filter(year == 2080) %>% 
  ungroup() %>% select(-c(year))
colnames(pres)[3] <- "pres"
colnames(fut)[3] <- "fut"

dat_wide <- full_join(pres, fut) %>% mutate(change = fut - pres)
dat_wide$year <- paste0(year[2], " - 1995")
lost_clim <- dat_wide %>% filter(is.na(fut)) %>% mutate(lost = "Lost climate")
novel_clim <- dat_wide %>% filter(is.na(pres)) %>% mutate(lost = "Novel climate")
...
p3 <- ggplot(data=dat_wide) + geom_tile(aes(x=bio1, y=bio12/100, fill=change)) + 
  ...
  scale_y_continuous(breaks=c(0, 20, 40, 60, 80), labels=c(0, 2000, 4000, 6000, 8000), 
                     expand=c(0,0)) + 
  scale_x_continuous(expand=c(0,0)) + coord_fixed(ratio=0.5) + 
...
ggsave("figures/FigureS20.png", p, width=9, height=9.7, dpi=350, bg="transparent")
