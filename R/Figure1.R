#' ---
#' title: "Spatial patterns in current and future reptile richness"
#' author: "RS-eco"
#' ---
...
library(scales); library(scico); library(ggsci)
...
min(tot_1995$mean); max(tot_1995$mean)
...
p1 <- ggplot() + geom_tile(data=dat_1995, aes(x=x, y=y, fill=mean)) + 
  ...
  scale_fill_gradientn(name="Species richness", colours=bluered, 
...
p2 <- ggplot() + 
  ...
  scale_x_sqrt(limits=c(0,NA), expand=c(0.01,0), 
               breaks=c(0,4,16,36,64,100,144,196,256), 
               labels=c(0,4,16,36,64,100,144,196,256)) + 
  ...
  ggtitle("b)") + theme_bw() + labs(x="Species richness (sqrt-scale)") + 
...
dat_dec <- dat %>% dplyr::select(c(x,y,species,algorithm,matches("rcp60_2080"),`EWEMBI_1995`)) %>% 
  ...
  mutate(change = if_else(change > 0, 0, change)) %>% ungroup() %>% 
  dplyr::select(-species) %>% group_by(x,y,algorithm,gcm) %>% summarise(decrease=sum(change)) %>% 
...
ggplot() + geom_tile(data=dat_dec %>% filter(mean < -50), aes(x=x, y=y, fill=mean)) + 
  geom_sf(data=outline_moll, fill=NA, color = "black", size = 1/.pt) +
...
ggplot() + geom_tile(data=dat_inc %>% filter(mean > 25), aes(x=x, y=y, fill=mean)) + 
  geom_sf(data=outline_moll, fill=NA, color = "black", size = 1/.pt) +
...
ggsave("figures/Figure1.png", p, width=9, height=10, dpi=600, bg="transparent")
