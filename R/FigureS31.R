...
# Files are created under Figure2.R
...
mean_dat <- r_dat %>% mutate(year = as.factor(year)) %>%
  group_by(year,realm) %>%
  dplyr::summarise(median_sr = median(value), 
                   mean_sr = round(mean(value),2),
                   sd = sd(value),
...
ggsave("figures/FigureS14.png", p2, dpi=300, width=9, height=11.6, bg="transparent")
