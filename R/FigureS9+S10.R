...
library(scico); library(ggpmisc); library(terra)
...
dat_dec <- dat %>% select(c(x,y,species,group,algorithm, matches("rcp60_2050"),`EWEMBI_1995`)) %>% 
  mutate_at(vars(matches("rcp60_2050")), list(~ . - `EWEMBI_1995`)) %>% select(-c(`EWEMBI_1995`)) %>% 
  mutate_at(vars(matches("rcp60_2050")), list(~ if_else(. > 0, 0, .))) %>% 
  ungroup() %>% select(-species) %>% group_by(x,y,group,algorithm) %>% 
  summarise_at(vars(matches("rcp60_2050")), sum); gc()
...
dat_dec <- dat_dec %>% spread(group, mean) %>% terra::rast()
...
dat_dec <- as.data.frame(r_dat_dec, xy=T) %>% dplyr::select(-lyr.1) %>%
...
p1 <- lapply(1:length(dat_dec), function(x){
  ...
  if(x == 1){
    p <- p + ggtitle("Species decrease (2050)") + 
...
dat_inc <- dat %>% select(c(x,y,species,group,algorithm, matches("rcp60_2050"),`EWEMBI_1995`)) %>% 
  mutate_at(vars(matches("rcp60_2050")), list(~ . - `EWEMBI_1995`)) %>% select(-c(`EWEMBI_1995`)) %>% 
  mutate_at(vars(matches("rcp60_2050")), list(~ if_else(. < 0, 0, .))) %>% 
  ungroup() %>% select(-species) %>% group_by(x,y,group,algorithm) %>% 
  summarise_at(vars(matches("rcp60_2050")), sum); gc()
...
p2 <- lapply(1:length(dat_inc), function(x){
  ...
  if(x == 1){
    p <- p + ggtitle("Species increase (2050)") + 
      theme(plot.title = element_text(hjust=0.5, size=12, face="bold"))
  }
  return(p)
})
...
ggsave("figures/FigureS9.png", p, width=9, height=9.8, dpi=400, bg="transparent")
...
data <- dat %>% select(c(x,y,species,group,algorithm, matches("rcp60_2050"),`EWEMBI_1995`)) %>% 
  mutate_at(vars(matches("rcp60_2050")), list(~ . - `EWEMBI_1995`)) %>% 
...
p3 <- lapply(1:length(dat_change), function(x){
  ...
  p <- dat_change[[x]] %>% ggplot() + geom_tile(aes(x=x, y=y, fill=sum)) + 
    ...
    theme(axis.title = element_blank(), axis.line = element_blank(), 
          ...
          strip.text = element_text(size=12, face="bold"))
  if(x == 1){
    p <- p + ggtitle("Net change to 1995 (2050)") + 
      theme(plot.title = element_text(hjust=0.5, size=12, face="bold"))
  }
  return(p)
})
...
data <- dat %>% select(c(x,y,species,group,algorithm, matches("rcp60_2050"),`EWEMBI_1995`)) %>% 
  group_by(x,y,group,algorithm) %>% summarise_at(vars(c(matches("rcp60_2050"),`EWEMBI_1995`)), sum) %>% 
  mutate_at(vars(matches("rcp60_2050")), list(~ (. - `EWEMBI_1995`)/`EWEMBI_1995`*100)) %>% 
...
p4 <- lapply(1:length(dat_change), function(x){
  ...
  p <- dat_change[[x]] %>% ggplot() + geom_tile(aes(x=x, y=y, fill=sum)) + 
    ...
    theme(axis.title = element_blank(), axis.line = element_blank(), 
          ...
          strip.text = element_blank())
  if(x == 1){
    p <- p + ggtitle("% change to 1995 (2050)") + 
      theme(plot.title = element_text(hjust=0.5, size=12, face="bold"))
  }
  return(p)
})
...
ggsave("figures/FigureS10.png", p, width=9, height=9.8, dpi=400, bg="transparent")
